...
func main() {
	...
	// ------------------------
	// 🧹 ОЧИСТКА ТАБЛИЦЫ USERS ПРИ СТАРТЕ
	_, err = db.Exec("TRUNCATE TABLE users RESTART IDENTITY;")
	if err != nil {
		log.Printf("⚠️ Не удалось очистить таблицу users: %v", err)
		// Можно заменить на log.Fatal, если очистка обязательна
	} else {
		log.Println("✅ Таблица users очищена при старте")
	}
